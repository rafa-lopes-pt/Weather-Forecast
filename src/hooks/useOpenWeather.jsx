...
const useOpenWeather = (lang = "en", units = "metric") => {
    ...
    const getForecast = async ({
        ...
        units = defaultUnits,
        lang = defaultLang,
        cnt = 40, //40 intervals of 3h = 5 Days <- 40 is the max value on free plan
    }) => {
        if (
            (typeof lat === "string" || typeof lat === "number") &&
            (typeof lon === "string" || typeof lon === "number") &&
            typeof units === "string" &&
            typeof lang === "string" &&
            (typeof cnt === "string" || typeof cnt === "number")
        ) {
            //parse params
            const params = `?lat=${lat}&lon=${lon}&units=${units}&lang=${lang}&cnt=${cnt}&${API_KEY}`;
